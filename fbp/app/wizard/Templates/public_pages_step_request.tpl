<form id="wizard_public_pages_request_form">
	{if $row.page_action == 'common_design'}
		<p style="font-size:13px;color:#374151;margin:0 0 8px 0;">public_pages 全体で使うヘッダ・フッタ・共通導線の要件を入力してください。</p>
	{elseif $row.page_action == 'edit'}
		<table class="moredata" style="margin-top:0px;margin-bottom:12px;">
			...
			<tr><th>function名</th><td><code style="display:inline;background:#111827;color:#fff;padding:4px 6px;border-radius:4px;font-size:10px;">{$row.function_name|escape}</code></td></tr>
			<tr><th>template</th><td>{$row.template_name|escape}</td></tr>
		</table>
		<p style="font-size:13px;color:#374151;margin:0 0 8px 0;">変更したい内容を入力してください。</p>
	{else}
		<table class="moredata" style="margin-top:0px;margin-bottom:12px;">
			<tr><th style="width:30%;">ページタイトル</th><td>{$row.title|escape}</td></tr>
			<tr><th>function名</th><td><code style="display:inline;background:#111827;color:#fff;padding:4px 6px;border-radius:4px;font-size:10px;">{$row.function_name|escape}</code></td></tr>
		...
		<p style="font-size:13px;color:#374151;margin:0 0 8px 0;">作りたい公開ページの内容を入力してください。</p>
	{/if}
	...
	{if count($selected_public_asset_rows) > 0}
		<p style="font-weight:bold;margin:12px 0 6px 0;">使用するPublic Assets</p>
	...
	<div style="margin-top:12px;overflow:auto;">
		<button type="button" class="ajax-link" invoke-function="back_to_public_pages_assets" style="float:left;">前へ</button>
		<button type="button" class="ajax-link" invoke-function="submit_public_pages_request_next" data-form="wizard_public_pages_request_form" style="float:right;">次へ</button>
	</div>
</form>
