...
- LINE user_id と会員DBを連結する場合は、`[getting_member]` / `match_type=data_type` を使う。
- `getting_member` は LINE の生イベントではなく、`webhook_line` が前処理として呼ぶ内部ルール。
...
- `LINE用会員データベース作製` は固定テンプレートとして扱い、上記のテーブル名・フィールド名で実装する。
